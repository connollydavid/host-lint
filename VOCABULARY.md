...
The signature of the flag list is a blocking noun from section 1 followed by a numeral (`Phase 1`, `Sprint 3`, `Stage II`, `Wave 2`) or a spelled ordinal (`Wave one`, `Phase two`). The numeral or the spelled ordinal is the actual tell; the noun alone is not. The spelled form catches the band-name shape a plan author reaches for (`wave one`, `wave two`, and the rest) that the arabic gate alone missed. The spelled set is closed and small, the cardinals and ordinals no higher than twenty, so it does not collide with ordinary prose. The framing is cross-model: GPT, Gemini, Claude, Cursor, and Copilot all produce numbered `Phase`/`Step`/`Part` headers, so this is not specific to one assistant. The domain-heavy nouns like `Step` warn rather than block (per §1b) and take only the numeral form, and the checklist nouns `steps`/`box`/`boxes` likewise take only the numeral form, because a spelled `steps one to six` is ordinary descriptive prose.
...
- **Ordinal-scaffold heading** — a markdown heading that is nothing but one noun and a one- or two-digit cardinal (`## Check 1`), the noun outside both term lists above: a position label with the vocabulary swapped out from under the lexical rules (host-lint#24). It stays advisory because the same shape carries genuine designators (`## Windows 11`): confirm one and declare it in the `LEXICON`, which masks it durably; under `# host-lint: strict` an undeclared hit escalates to a flag whose remedy names the declaration. A heading with more words, a dotted or three-or-more-digit number, or a noun the term lists already judge is not this rule's business.
- **Bare dotted code** — a standalone `N.N` token used as a name (`as decided in 2.1`, `exec tools (5.5)`, `the tools arrive in 5.3`). A token carrying a letter (`v2.1`) is a version and is skipped; a trailing `%` or a following unit (`5.5 seconds`) marks a quantity and is skipped; a preceding version or cross-reference word (`python 3.11`, `figure 2.1`) is skipped. A preceding **all-caps designator** of two or more letters (`NT 3.1`, `SDK 2.1`, `DOS 6.2`) reads as a product or version name and is skipped, while a Title-case or lowercase noun before the decimal (`Decision 2.1`, `in 2.1`) still warns. A numeral in a **data position** is also skipped (host-lint#21): one fused to or flanked by pipe delimiters (a markdown table cell, where the unit sits in the column header on another line); one carrying an approximation or comparison operator (`≈ 1.0`, `> 67.5`, or a trailing multiplier `2.2×`); one followed by a slashed unit (`11.34 t/s`, `3.2 ms/token`); or one followed by a domain unit the project declares via the `# host-lint: unit` directive (clearing `312.4 GFLOPS`). These are measurement values, not names; the noun-gated flag tier is unaffected, so a real tell (`Phase 2.1`) still flags inside a table cell. The warn is deliberately recall-biased — it will still warn on some version or figure references (`upgrade to 2.1`), which is acceptable because it only asks the author to reconsider.
...
A declaration for **scope** (host-lint#26): `# host-lint: root` marks a LEXICON as the top of its own scope. The lexicon governing a document is resolved from that document's directory outward rather than from the invocation, so a vendored repository inside another answers to its own vocabulary; phrases accumulate outward because an allowlist is additive, `strict` holds if any scope declares it, and `root` is the only way to stop inheriting.

A declaration for **domain measurement units** (host-lint#21): `# host-lint: unit <TOKEN>` in the LEXICON declares a unit token (`GFLOPS`, `ppm`, `dB`) so a bare dotted code followed by it reads as a quantity, not a name. Unlike a phrase entry it is not masked; it extends the unit set the bare-dotted-code rule consults. A unit is a single token; a slashed compound (`t/s`, `ms/token`) is recognised structurally and needs no declaration.
