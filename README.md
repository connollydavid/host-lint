...
The lexicon governing a document is resolved from **its own directory outward**, not from wherever the command was run: the search walks up to the tree under audit, and phrases accumulate, because an allowlist is additive and a nearer scope adds to what encloses it rather than replacing it. `strict` holds if any scope in the chain declares it, so an inner scope can never quietly relax an outer escalation, and `# host-lint: root` is the only way to stop inheriting. This is what lets a vendored repository keep its own vocabulary: before it, linting an inner repository's document from the outer one masked it with the outer project's phrases and reported a defect belonging to neither (host-lint#26).

Each entry's phrase is masked out of every line before detection, case-insensitively and at word boundaries, so an occurrence of that exact phrase never flags — anywhere in the repo, in any mode: the naming modes (`--stdin`, files, `--all`, `--log`) and the prose lanes (`--prose`, `--docs`), where the phrase is masked before the host-grammar density engine as well. Only the phrase is masked; a trailing `http(s)://…` token is recorded as provenance, not masked. The boundary requirement keeps an entry specific: declaring `phase 1` clears `phase 1` but not the longer tell `phase 12`, and a *different* tell on the same line still flags (`section 1 covers phase 4` still reports `phase 4`). A comment is `#` followed by a non-digit (so `# note` and `## heading` are comments, while `#7 …` is a hash-number entry); blank lines are ignored. No file means no lexicon — the feature is opt-in per repo.
...
**`# host-lint: root`** — this LEXICON is the top of its own scope, so the search that found it stops rather than continuing outward. A repository can contain another one (a vendored template, a submodule, a subtree), and its documents answer to its own vocabulary; without `root` they would also inherit the enclosing project's phrases, legitimizing a token the inner repository never declared.
